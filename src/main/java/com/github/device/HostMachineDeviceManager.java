...
import com.github.manager.GenyMotionManager;
import com.github.manager.IAppiumManager;
import com.github.utils.Api;
import com.github.utils.AvailablePorts;
import com.github.utils.OSType;
import com.github.utils.TestStatusManager;
...
public class HostMachineDeviceManager {
        ...
        if (udids.size() == 0) {
            ...
            devicesByHost.forEach((hostName, appiumDevices) -> {
                List<AppiumDevice> filteredDevices = appiumDevices.stream()
                    .filter(appiumDevice -> udids.contains(appiumDevice.getDevice()
                        .getUdid()))
    ...
    private Map<String, List<AppiumDevice>> filterByDevicePlatform(Map<String, List<AppiumDevice>> devicesByHost) {
        ...
        if (platform.equalsIgnoreCase(OSType.BOTH.name())) {
            ...
            devicesByHost.forEach((hostName, appiumDevices) -> {
                List<AppiumDevice> filteredDevices = appiumDevices.stream()
                    .filter(appiumDevice -> appiumDevice.getDevice()
                        .getOs()
    ...
    private List<AppiumDevice> getDevicesByIP(String ip, String platform, JSONObject hostMachineJson) throws Exception {
        ...
        if ((!platform.equalsIgnoreCase(
            "android") && capabilityManager.isSimulatorAppPresentInCapsJson() && hostMachineJson.has(
            "simulators")) && !capabilityManager.getCapabilityObjectFromKey("iOS")
            .has("browserName")) {
            JSONArray simulators = hostMachineJson.getJSONArray("simulators");
            List<Device> simulatorsToBoot = getSimulatorsToBoot(ip, simulators);
            ...
            for (Object hostMachine : hostMachines) {
                ...
                if (machineIPs instanceof JSONArray) {
                    ...
                    if (capabilityManager.isCloud(ip)) {
                        ...
                        cloud.toMap()
                            .forEach((devicePlatform, devices) -> {
                                ((List) devices).forEach(o -> {
                                    Device d = new Device();
                                    d.setOs(devicePlatform);
                                    d.setOsVersion(((Map) o).get("osVersion")
                                        .toString());
                                    d.setName(((Map) o).get("deviceName")
                                        .toString());
                                    d.setCloud(true);
                                    device.add(d);
                                });
