...
public class SessionContext {
    ...
    private static Properties loadReportPortalProperties() {
        ...
        try {
            ...
            if (System.getenv()
                .containsKey("REPORT_PORTAL_FILE")) {
                reportPortalPropertiesFile = System.getenv()
                    .get("REPORT_PORTAL_FILE");
            }
    ...
    public static void setReportPortalLaunchURL(ITestContext iTestContext) {
        Optional reportPortalListener = Arrays.stream(iTestContext.getSuite()
            .getXmlSuite()
            .getListeners()
            .toArray())
            .filter(x -> x.equals("com.epam.reportportal.testng.ReportPortalTestNGListener"))
            .findFirst();
        boolean isReportPortalEnabledInProperties = (null == reportPortalProperties.getProperty(
            "rp.enable") || (reportPortalProperties.getProperty("rp.enable")
