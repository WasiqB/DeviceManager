...
public class Api extends Helpers {
    ...
    public String uploadMultiPartFile(File filePath, String hostMachine) throws Exception {
        ...
        RequestBody requestBody = new MultipartBody.Builder().setType(MultipartBody.FORM)
            .addFormDataPart("uploaded_file", filePath.getName(), RequestBody.create(MEDIA_TYPE_PNG, filePath))
            .build();
        Request request = new Request.Builder().url(
            "http://" + hostMachine + ":" + getRemoteAppiumManagerPort(hostMachine) + "/artifacts/upload")
            .post(requestBody)
            .build();
