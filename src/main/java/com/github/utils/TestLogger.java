...
import com.github.annotations.Author;
import com.github.manager.AppiumDeviceManager;
import com.github.manager.AppiumDriverManager;
import com.github.manager.ScreenShotManager;
import com.github.video.recorder.AppiumScreenRecordFactory;
import com.github.video.recorder.IScreenRecord;
...
public class TestLogger extends Helpers {
    ...
    public void startLogging(ITestResult iTestResult) throws IOException, InterruptedException {
        String methodName = iTestResult.getMethod()
            .getMethodName();
        ...
        if (isNativeAndroid()) {
            String udid = AppiumDeviceManager.getAppiumDevice()
                .getDevice()
                .getUdid();
            List<LogEntry> logcat = AppiumDriverManager.getDriver()
                .manage()
                .logs()
                .get("logcat")
                .filter(Level.ALL);
    ...
    private void setDescription(ITestResult iTestResult) {
        ...
        String description = "Platform: " + AppiumDeviceManager.getMobilePlatform() + " UDID: " + AppiumDeviceManager.getAppiumDevice()
            ...
            .getDevice()
            .getName() + " Host: " + AppiumDeviceManager.getAppiumDevice()
            .getHostName();
        Author annotation = iTestResult.getMethod()
        ...
        if (originalDescription.isPresent() && !originalDescription.get()
            .contains(AppiumDeviceManager.getAppiumDevice()
                .getDevice()
                .getUdid())) {
            iTestResult.getMethod()
                .setDescription(originalDescription.get() + "\n" + description);
        } else {
            iTestResult.getMethod()
    ...
    public HashMap<String, String> endLogging(ITestResult result, String deviceModel)
        ...
        if (isNativeAndroid()) {
            String adbPath = System.getProperty(
                "user.dir") + FileLocations.ADB_LOGS_DIRECTORY + AppiumDeviceManager.getAppiumDevice()
                .getDevice()
                .getUdid() + "__" + result.getMethod()
                .getMethodName() + ".txt";
        ...
        String baseHostUrl = "http://" + getHostMachineIpAddress() + ":" + getRemoteAppiumManagerPort(
            AppiumDeviceManager.getAppiumDevice()
                .getHostName());
        if ("true".equalsIgnoreCase(System.getenv("VIDEO_LOGS"))) {
            setVideoPath("screenshot/" + AppiumDeviceManager.getMobilePlatform()
                .toString()
                .toLowerCase() + "/" + AppiumDeviceManager.getAppiumDevice()
                .getDevice()
                .getUdid() + "/" + className + "/" + result.getMethod()
        ...
        if (result.getStatus() == ITestResult.FAILURE) {
            ...
            if (new File(screenFailure).exists()) {
                ...
                String framedScreenFailure = System.getProperty(
                    "user.dir") + FileLocations.OUTPUT_DIRECTORY + framedFailureScreen;
                if (new File(framedScreenFailure).exists()) {
                    screenShotFailure = screenShotFailure + "/" + framedScreenFailure;
                    logs.put("screenShotFailure", screenShotFailure);
                }
            }
        }
    ...
    private boolean isNativeAndroid() {
        return AppiumDeviceManager.getMobilePlatform()
            .equals(MobilePlatform.ANDROID) && AppiumDriverManager.getDriver()
            .getCapabilities()
        ...
        if (result.getStatus() == ITestResult.FAILURE) {
            ...
            if (AppiumDeviceManager.getMobilePlatform()
                ...
                String imagePath = System.getProperty(
                    ...
                    .getDevice()
                    .getUdid() + "/" + className + "/" + result.getMethod()
                    .getMethodName() + "/" + screenShotNameWithTimeStamp + "-" + result.getMethod()
                    .getMethodName() + "_failed.jpeg";
                ReportPortal.emitLog("Screenshots", "ERROR", new Date(), new File(imagePath));
            }
            if (AppiumDeviceManager.getMobilePlatform()
                .equals(MobilePlatform.IOS)) {
                String imagePath = System.getProperty(
                    "user.dir") + FileLocations.IOS_SCREENSHOTS_DIRECTORY + AppiumDeviceManager.getAppiumDevice()
                    .getDevice()
                    .getUdid() + "/" + className + "/" + result.getMethod()
                    .getMethodName() + "/" + screenShotNameWithTimeStamp + "-" + result.getMethod()
                    .getMethodName() + "_failed.jpeg";
