        ...
        for (File file : files) {
            if (file.isDirectory()) {
                ...
                if (file.getAbsolutePath()
                    .contains(deviceUDID) && file.getAbsolutePath()
                    .contains(getCurrentTestMethodName())) {
                        ...
                        if (atdHost.isPresent() && atdPort.isPresent()) {
                            list.put(screenName,
                                "http://" + getHostMachineIpAddress() + ":" + getRemoteAppiumManagerPort(
                                    "127.0.0.1") + path);
                            ReportPortal.emitLog(screenName, "Info", new Date(),
                                new File(System.getProperty("user.dir") + "/target/" + path));
                        } else {
                            ReportPortal.emitLog(screenName, "Info", new Date(),
                                new File(System.getProperty("user.dir") + "/target/" + path));
                        }
