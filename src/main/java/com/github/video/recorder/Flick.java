...
public class Flick extends CommandPrompt implements IScreenRecord {
    ...
    private void flickRecordingCommand(String command, String className, String methodName,
        ...
        if (AppiumDeviceManager.getAppiumDevice()
            ...
            String videoLocationAndroid = videoPath + FileLocations.ANDROID_SCREENSHOTS_DIRECTORY + AppiumDeviceManager.getAppiumDevice()
                .getDevice()
                .getUdid() + "/" + className + "/" + methodName;
            fileDirectoryCheck(videoLocationAndroid);
            if (command.equals("start")) {
                try {
                    android = "flick video -a " + command + " -p android -u " + AppiumDeviceManager.getAppiumDevice()
                        .getDevice()
                        .getUdid();
                ...
                android = "flick video -a " + command + " -p android -o " + videoLocationAndroid + " -n " + videoFileName + " -u " + AppiumDeviceManager.getAppiumDevice()
                    .getDevice()
                    .getUdid() + " --trace";
                runCommandThruProcess(android);
            ...
            if (command.equals("start")) {
                ios = "flick video -a " + command + " -p ios -u " + AppiumDeviceManager.getAppiumDevice()
                    .getDevice()
                    .getUdid();
            } else {
                ios = "flick video -a " + command + " -p ios -o " + videoLocationIOS + " -n " + videoFileName + " -u " + AppiumDeviceManager.getAppiumDevice()
                    .getDevice()
                    .getUdid();
            }
