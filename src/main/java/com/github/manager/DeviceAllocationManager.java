...
public class DeviceAllocationManager extends ArtifactsUploader {
    ...
    public List<AppiumDevice> getDevices() {
        ...
        return hostMachineDeviceManager.getDevicesByHost()
            .getAllDevices();
    }
    ...
    public synchronized void freeDevice() {
        AppiumDeviceManager.getAppiumDevice()
            .freeDevice();
        LOGGER.info("DeAllocated Device " + AppiumDeviceManager.getAppiumDevice()
            .getDevice()
            .getUdid() + " from execution list");
        if (suspendedThreads.size() > 0) {
            suspendedThreads.get(0)
                .resume();
