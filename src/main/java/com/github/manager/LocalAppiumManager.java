...
public class LocalAppiumManager implements IAppiumManager {
    ...
    public void startAppiumServer(String host) throws Exception {
        ...
        AppiumDriverLocalService appiumDriverLocalService;
        AppiumServiceBuilder builder = getAppiumServerBuilder(host).withLogFile(new File(
            System.getProperty(
                "user.dir") + FileLocations.APPIUM_LOGS_DIRECTORY + "appium_logs.txt"))
            .withIPAddress(host)
        ...
        LOGGER.info(
            LOGGER.getName() + "Appium Server Started at......" + appiumDriverLocalService.getUrl());
        ...
        if (platform.equalsIgnoreCase(OSType.iOS.name()) || platform.equalsIgnoreCase(
            ...
            if (CapabilityManager.getInstance()
                ...
                if (CapabilityManager.getInstance()
                    ...
                    devices.addAll(
                        new SimulatorManager().getAllBootedSimulators(OSType.iOS.name()));
                }
                if (CapabilityManager.getInstance()
    ...
    public int startIOSWebKitProxy(String host) throws Exception {
        ...
        String webkitRunner = "ios_webkit_debug_proxy -c " + AppiumDeviceManager.getAppiumDevice()
            .getDevice()
            .getUdid() + ":" + port;
        String process = Runtime.getRuntime()
            .exec(webkitRunner)
            .toString();
        AppiumDeviceManager.getAppiumDevice()
            .setWebkitProcessID(process);
        return port;
    }
    ...
    public void destoryIOSWebKitProxy(String host) throws Exception {
        if (AppiumDeviceManager.getAppiumDevice()
            ...
            String command = "kill -9 " + AppiumDeviceManager.getAppiumDevice()
                .getWebkitProcessID();
    ...
    private AppiumServiceBuilder getAppiumServiceBuilderWithUserAppiumPath(String host)
        throws Exception {
        return new AppiumServiceBuilder().withAppiumJS(new File(CapabilityManager.getInstance()
