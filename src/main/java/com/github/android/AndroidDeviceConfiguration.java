...
import com.github.device.DevicesByHost;
...
public class AndroidDeviceConfiguration {
    ...
    public String getDeviceModel() {
        ...
        return (getModel.getDevice()
            .getDeviceModel() + getModel.getDevice()
            .getBrand()).replaceAll("[^a-zA-Z0-9\\.\\-]", "");
    }
    ...
    public String screenRecord(String fileName) {
        return "adb -s " + AppiumDeviceManager.getAppiumDevice()
            .getDevice()
            .getUdid() + " shell screenrecord --bit-rate 3000000 /sdcard/" + fileName + ".mp4";
    }

    public boolean checkIfRecordable() throws IOException, InterruptedException {
        String screenrecord = cmd.runCommand("adb -s " + AppiumDeviceManager.getAppiumDevice()
            .getDevice()
            .getUdid() + " shell ls /system/bin/screenrecord");
        if (screenrecord.trim()
            .equals("/system/bin/screenrecord")) {
            return true;
        } else {
            return false;
        }
    ...
    public String getDeviceManufacturer() {
        return devicesByHost.getDeviceProperty(AppiumDeviceManager.getAppiumDevice()
            .getDevice()
            .getUdid())
            .getDevice()
    ...
    public AndroidDeviceConfiguration pullVideoFromDevice(String fileName, String destination)
        ...
        ProcessBuilder pb = new ProcessBuilder("adb", "-s", AppiumDeviceManager.getAppiumDevice()
            .getDevice()
            .getUdid(), "pull", "/sdcard/" + fileName + ".mp4", destination);
        Process pc = pb.start();
        pc.waitFor();
    ...
    public void removeVideoFileFromDevice(String fileName)
        throws IOException, InterruptedException {
        cmd.runCommand("adb -s " + AppiumDeviceManager.getAppiumDevice()
            .getDevice()
            .getUdid() + " shell rm -f /sdcard/" + fileName + ".mp4");
    }
